...
\Prop{}
Seien $L|K$ galoissch und $\Pf|\pf$ unverzweigte Stellen in $\O_L|\O_K$. Es bezeichne $\lambda = \O_L / \Pf$ und $\kappa = \O_K /\pf$ die korrespondierenden Restklassenkörper. Dann ist $G_\Pf := G(\lambda | \kappa) \inj{\iota} G(L|K)$ zyklisch und wird vom \df{Frobeniusautomorphismus}
\begin{align*}
...
\draw[->] (D3) -> (D7) node[midway, right]{$\sigma^* : g \mapsto \sigma\i g \sigma$};
...
\draw[->] (D1) -> (D5) node[midway, left]{$N_{K'|K}$};
\end{tikzpicture}
\end{center}
...
\draw[->] (D5) -> (D7) node[midway, above]{$\phi_K : \overline{a} \mapsto \Frob_v$};
\draw[right hook->] (D3) -> (D7) node[midway, right]{};
\draw[right hook->] (D1) -> (D5) node[midway, left]{$\pi_v \mapsto \overline{a}$};
\end{tikzpicture}
\end{center}
wobei $K_v$ die Komplettierung von $K$ bzgl. $v$ bezeichnet und $a = (\ldots, 1, \pi_v, 1,\ldots) \in K_v^\times \times \prod_{v' \neq v}\O_{K_v}^\times \subset_o \A_K^\times $ 
