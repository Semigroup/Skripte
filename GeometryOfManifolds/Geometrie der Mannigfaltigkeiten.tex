...
%Das folgende Kurzskript orientiert sich an einer Vorlesung, 
%die im Wintersemester 2016 / 2017 in Heidelberg gehalten wurde. 
%Für alle Fehler im Text trägt ausschließlich der Autor die Verantwortung.
...
\begin{itemize}
	\item \df{zeitartig}, falls $\shrp{x,x} < 0$,
	\item \df{lichtartig}, falls $\shrp{x,x} = 0$,
	\item \df{raumartig}, falls $\shrp{x,x} > 0$.
\end{itemize}
...
\begin{enumerate}[1.)]
\item \df{Horizontale Translationen}
\begin{align*}
x \longmapsto x + (b_1,\ldots, b_{n-1}, 0)
\end{align*}
\item \df{Dilationen} für $\lambda > 0$
\[ x \longmapsto \lambda \cdot x \]
\item \df{Inversionen} an Sphären orthogonal zu $\partial H^n$.
\end{enumerate}
...
\begin{itemize}
	...
	\item $S$ und $S'$ sind \df{asymptotisch parallel}, d.\,h., $S\cap S' = \emptyset$ und $d(S, S') = 0$. Ferner ist dann $\overline{S} \cap \overline{S'}$ ein Punkt in $\H^n$ und es existiert keine Geodäte, die zu beiden Räumen orthogonal ist.
...
\Def{}
Es bezeichne $S = \C \cup \{\infty\}$ die \df{Riemannsche Zahlenkugel}. Die Gruppe $PSL_2(\C)$ agiert auf $S$ durch \df{Möbiustransformationen}
\[ \klam{
\begin{matrix}
a & b\\
c & d
...
U_{(k)} &:= [U_{(k-1)}, U]
\end{align*}
Ist $U$ eine Umgebung der Eins in einer Lie-Gruppe $G$, so nennen wir $U$ eine \df{Zassenhaus-Umgebung}, falls
\begin{align*}
U_{(1)} &\subset U\\
...
\begin{itemize}
	...
	\item Eine \df{sternförmige Umgebung um eine Gerade} $l \subset \H^n$ ist eine Umgebung $U \subset \H^n$, die einen nicht-leeren zusammenhängenden Schnitt mit jeder Geodäte besitzt, die $l$ orthogonal schneidet.
	\item Sei $\Gamma$ eine Gruppe, die frei und diskontinuierlich agiert und aus parabolischen Isometrien besteht, die einen Randpunkt $p\in \partial\H^n$ fixieren. Ist $U$ eine sternförmige Umgebung um $p$, die invariant unter $\Gamma$ ist, so heißt $U / \Gamma$ eine \df{sternförmige Spitzenumgebung}.
...
\begin{Beweis}{}
	\begin{itemize}
		...
		eine Übderdeckung von $S_0$ durch geodätische Halbstrahlen, die jeweils \df{Blätter} der Blätterung genannt werden. $\Gamma_0$ erhält diese Blätterung, weswegen wir eine Blätterung in $M_0$ erhalten.
...
\Def{}
Eine Gruppe $G$ agiere auf einem topologischen Raum $X$. Eine abgeschlossene, nicht-leere, $G$-invariante Menge $A \subset X$ heißt \df{minimal}, falls keine abgeschlossene, nicht-leere, $G$-invariante Teilmenge $B \subsetneq A$ existiert.
...
\[ d(f(w),z) \leq D \]
gilt. In einem solchen Fall heißen $X,Y$ \df{quasi-isometrisch} zueinander.
...
gilt. In einem solchen Fall heißen $X,Y$ \df{pseudo-isometrisch} zueinander.\\
...
\begin{Beweis}{} %%% Funktioniert das wirklich?
...
\Def{}
Sei $\alpha : \Delta_k \pfeil{} M$ ein gerader Simplex. Definiere das \df{abstrakte Volumen} von $\alpha$ durch seinen Lift in $\H^n$ durch
