...
\paragraph{Korrekte Definition:}
Definiere den Cayley-Komplex $K(S,R)$ als universelle Überlagerung von $K_G$.\\
\paragraph{Explizite Beschreibung des Cayleykomplexes}
$\grp{S}{R} = G$, $S$ reduziert.\\
Sei $\Gamma$ der Graph, der aus dem $Cay(G,S)$ durch folgende Operation entsteht:
\begin{enumerate}
	\item $\forall s \in S$ mit $s^2 = 1$, füge $\forall g$ eine zweite Kante der Form $(g,gs)$ hinzu.
	\item Orientiere alle Kanten $(g,gs)$ im Graphen so, dass sie von $g$ nach $gs$ zeigen\\
	Ordnung 2 Elemente liefern Doppelkanten zu zwei Punkten. $\overset{\curvearrowright}{\longleftarrow}$
	
	\item 
	klebe 2-Zelle $d_{r,g}$ an $\Gamma$ für alle Paare $(r,g) \in R \times G $, $G \sim V(\Gamma)$ wie folgt:\\
	verklebe 1 mit $g \in V(\Gamma)$ und den Rand von $d_{r,g}$ mit dem Pfad (eind.), der zum Wort $r$ gehört, das in $g$ startet.
\end{enumerate}
...
Ein \df{Unterstrahl} eines Strahles $(x_i)_{i \geq 0}$ ist ein Strahl $(y_j)_{j\geq 0}$ mit $y_i = x_i+k$ für alle $i$, $k\in \N$ fest.\\
Zwei Strahlen $(x_i)_{i\geq 0}, (y_j)_{j\geq 0}$ werden von einer endlichen Menge $S$ \df{separiert}, wenn es zwei disjunkte Unterstrahlen $(x'_i), (y'_j)$ in $V-S$ gibt, deren Eckenmengen von $S$ separiert werden. 
